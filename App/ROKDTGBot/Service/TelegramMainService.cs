...
namespace ROTGBot.Service
{
    public class TelegramMainService : ITelegramMainService
    {
        ...
        private async Task<bool> HandleCallback(TelegramBotClient client, CallbackQuery? callbackQuery, CancellationToken token)
        {
            ...
            {
                user = await _userDataService.GetOrAddUser(tgUser, null, token);
            }
            else
            {
                user = await _userDataService.GetOrAddUser(tgUser, chatId.Value, token);
            }
            if (user == null)
            {
                return false;
            ...
            CancellationToken token)
        {
            ...
            return data switch
            {
                ...
                "UserReport" => await SendWithCheckRights(client, user, chatId.Value, RoleEnum.user,
                                        (cl, chId, userNews, tk) => GetUserReportHandle(cl, chId, user, tk), token),
                "ModeratorReport" => await SendWithCheckRights(client, user, chatId.Value, RoleEnum.user,
                ...
                "ViewUserRolesChoice" => await SendWithCheckRights(client, user, chatId.Value, RoleEnum.administrator,
                ...
                "ViewUserRoles" => await SendWithCheckRights(client, user, chatId.Value, RoleEnum.administrator,
                                        (cl, chId, userNews, tk) => SendViewUserRolesHandle(cl, chId, user, userNews, tk), token),
                "ViewUserRolesDecline" => await SendWithCheckRights(client, user, chatId.Value, RoleEnum.administrator,
                                        (cl, chId, userNews, tk) => SendViewUserRolesDeclineHandle(cl, chId, user, userNews, tk), token),
                "ViewDemandUsers" => await SendWithCheckRights(client, user, chatId.Value, RoleEnum.administrator,
                                        (cl, chId, userNews, tk) => SendViewDemandUsersHandle(cl, chId, user, userNews, tk), token),
                "ApproveUserDemand" => await SendWithCheckRights(client, user, chatId.Value, RoleEnum.administrator,
                                        (cl, chId, userNews, tk) => SendApproveUserDemandHandle(cl, chId, user, userNews, userIdn, tk), token),
                ...
                "DeleteAdminChoice" => await SendWithCheckRights(client, user, chatId.Value, RoleEnum.administrator,
                                        (cl, chId, userNews, tk) => SendDeleteAdminChoiceHandle(cl, chId, user, userNews, tk), token),
                "DeleteModeratorChoice" => await SendWithCheckRights(client, user, chatId.Value, RoleEnum.administrator,
                                        (cl, chId, userNews, tk) => SendDeleteModeratorChoiceHandle(cl, chId, user, userNews, tk), token),
                "BlockUserChoice" => await SendWithCheckRights(client, user, chatId.Value, RoleEnum.administrator,
                                        (cl, chId, userNews, tk) => SendBlockUserChoiceHandle(cl, chId, user, userNews, tk), token),
                "UnBlockUserChoice" => await SendWithCheckRights(client, user, chatId.Value, RoleEnum.administrator,
                                        (cl, chId, userNews, tk) => SendUnBlockUserChoiceHandle(cl, chId, user, userNews, tk), token),
                "EditButtonChoice" => await SendWithCheckRights(client, user, chatId.Value,  RoleEnum.administrator,
                ...
                "DeleteAdmin" => await SendWithCheckRights(client, user, chatId.Value, RoleEnum.administrator,
                                        (cl, chId, userNews, tk) => DeleteAdminHandle(cl, userId, chId, userNews, tk), token),
                "DeleteAdminDecline" => await SendWithCheckRights(client, user, chatId.Value, RoleEnum.administrator,
                                        (cl, chId, userNews, tk) => DeleteAdminDeclineHandle(cl, userId, chId, userNews, tk), token),
                "DeleteModerator" => await SendWithCheckRights(client, user, chatId.Value, RoleEnum.administrator,
                                        (cl, chId, userNews, tk) => DeleteModeratorHandle(cl, userId, chId, userNews, tk), token),
                "DeleteModeratorDecline" => await SendWithCheckRights(client, user, chatId.Value, RoleEnum.administrator,
                                        (cl, chId, userNews, tk) => DeleteModeratorDeclineHandle(cl, userId, chId, userNews, tk), token),
                "BlockUser" => await SendWithCheckRights(client, user, chatId.Value, RoleEnum.administrator,
                ...
                "AddButtonDecline" => await SendWithCheckRights(client, user, chatId.Value, RoleEnum.administrator,
                                        (cl, chId, userNews, tk) => AddtButtonDeclineHandle(cl, userId, chId, userNews, tk), token),
        ...
        private async Task SendViewUserRolesHandle(TelegramBotClient client, long chatId, Contract.Model.User user, News? userNews, CancellationToken token)
        {
            if (userNews != null)
            {
                await SendViewUserRoles(client, chatId, userNews, token);
            }
            else
            {
                await ViewUserRolesMessageNotFound(client, chatId);
            }
        }

        private async Task ViewUserRolesMessageNotFound(TelegramBotClient client, long chatId)
        {
            throw new NotImplementedException();
        }
        ...
        private static string? GetButtonsView(List<NewsButton> availableButtons, int level = 0)
        {            
            if (!availableButtons.Any())
                return null;

            return string.Join("\n", availableButtons.OrderBy(s => s.ButtonNumber)
                .Select(s => GetButtonName(s, true)));
        }
        ...
        private async Task SendViewUserRolesChoiceHandle(TelegramBotClient client, long chatId, Contract.Model.User user, News? userNews, CancellationToken token)
        {
            if (userNews != null)
            {
                await SendUserRemember(client, chatId, userNews, token);
            ...
            {
                await SendViewUserRolesForUser(client, chatId, user, token);
            }
        }
        ...
        private async Task AddButtonAccepted(TelegramBotClient client, Guid moderatorId, long chatId, News userNews, CancellationToken token)
        {
            ...
            var settings = ParseButtonSettings(messages.FirstOrDefault());

            if (settings == null)
            {
                await client.SendMessageAsync(chatId, "Не отправлено ни одной кнопки либо неверный формат сообщения", cancellationToken: token);
                return;
            }
            ...
            if (button == null)
            {
                await client.SendMessageAsync(chatId, $"Не найдена кнопка по номеру ({settings.Number})", cancellationToken: token);
                return;
            }
        ...
        private async Task SendViewUserRoles(TelegramBotClient client, long chatId, News userNews, CancellationToken token)
        {
            var messages = await _newsDataService.GetNewsMessages(userNews.Id, token);
            ...
            Contract.Model.User user = await _userDataService.GetUserByLoginOrNumber(login, token);
            var roles = user.Roles.Select(s => {
                return s switch
                {
            ...
            await client.SendMessageAsync(chatId, $"Права пользователя {user.Number}:{user.TGLogin}: {string.Join(";", roles)}", cancellationToken: token);           
        }
        ...
        private static ButtonSetting? ParseButtonSettings(NewsMessage? message)
        {
            if (message?.TextValue == null)
                return null;

            var itemElements = message.TextValue.Split(":").Select(s => s.Trim()).ToArray();
            if (!int.TryParse(itemElements[0], out int num))
            {
                return null;
            }
        ...
        private async Task BlockUserDecline(TelegramBotClient client, Guid moderatorId, long chatId, News userNews, CancellationToken token)
        {
            await _newsDataService.SetNewsDeclined(userNews.Id, moderatorId, token);
            await client.SendMessageAsync(chatId, "Задание отменено", cancellationToken: token);
        }
        ...
        private async Task BlockUserAccepted(TelegramBotClient client, Guid moderatorId, long chatId, News userNews, CancellationToken token)
        {
            try
            {
                var messages = await _newsDataService.GetNewsMessages(userNews.Id, token);
                ...
                var user = await _userDataService.GetUserByLoginOrNumber(login, token);
                if (user == null)
                {
                ...
                await client.SendMessageAsync(chatId, "Пользователь заблокирован", cancellationToken: token);
            }
            catch (Exception ex)
            {
                await client.SendMessageAsync(chatId, $"Ошибка при блокировке пользователя: {ex.Message}", cancellationToken: token);
            }
        }
        ...
        private static async Task BlockUserMessageNotFound(TelegramBotClient client, long chatId)
        {
            await client.SendMessageAsync(chatId, "Нет задач на блокировку пользователя");
        }
        ...
        private async Task SendBlockUserChoice(TelegramBotClient client, long chatId, Contract.Model.User user, CancellationToken token)
        {
            await _newsDataService.CreateNews(chatId, user.Id, null, null, "blockuser", "Блокировка пользователя", token);

            var button1 = new InlineKeyboardButton("Отменить")
            {
                CallbackData = "BlockUserDecline"
            };
            ReplyMarkup replyMarkup = new InlineKeyboardMarkup(
                new List<List<InlineKeyboardButton>>()
                {
            ...
            await client.SendMessageAsync(chatId, "Отправьте номер или логин пользователя для блокировки или нажмите кнопку Отменить для отмены",
                replyMarkup: replyMarkup,
                cancellationToken: token);
        }
        ...
        private async Task SendViewUserRolesForUser(TelegramBotClient client, long chatId, Contract.Model.User user, CancellationToken token)
        {
            await _newsDataService.CreateNews(chatId, user.Id, null, null, "viewuserroles", "Просмотр прав пользователя", token);

            var button1 = new InlineKeyboardButton("Отменить")
            {
                CallbackData = "ViewUserRolesDecline"
            };
            ReplyMarkup replyMarkup = new InlineKeyboardMarkup(
                new List<List<InlineKeyboardButton>>()
                {
            ...
            await client.SendMessageAsync(chatId, "Отправьте логин или номер пользователя для просмотра его прав или нажмите Отмена для отмены задачи",                
                cancellationToken: token);
        }
        ...
        private static List<List<InlineKeyboardButton>> GetAdminButtons()
        {
            ...
            [
                ...
                [
                    new InlineKeyboardButton("Просмотр прав пользователя")
                    {
                        CallbackData = "ViewUserRolesChoice"
                    }
                ]
            ];
        }
